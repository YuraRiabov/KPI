        ...
        while (!escape)
        {
            ...
            switch (input)
            {
                ...
                case "15":
                    ...
                    break;
                case "16":
                    Console.WriteLine("Get people with minimum payment:");
                    foreach (var (person, minPayment) in _personService.GetPeopleMinPayment())
                    {
                        Console.WriteLine($"{person.FirstName} {person.LastName}: {minPayment:C}");
                    }
                    break;
                case "17":
                    Console.WriteLine("Get average number of degrees per person:");
                    double averageDegreeNumber = _personService.GetAverageDegreeNumber();
                    Console.WriteLine($"Average number of degrees per person: {averageDegreeNumber}");
                    break;
                case "18":
                    Console.WriteLine("Get payment count per person:");
                    foreach (var (person, paymentCount) in _personService.GetPeoplePaymentCount())
                    {
                        Console.WriteLine($"{person.FirstName} {person.LastName}: {paymentCount}");
                    }
                    break;
                case "19":
                    Console.WriteLine("Get average payment for philosophy doctors:");
                    decimal averagePhilosophyDoctorPayment = _personService.GetAveragePhilosophyDoctorPayment();
                    Console.WriteLine($"Average payment for philosophy doctors: {averagePhilosophyDoctorPayment}");
                    break;
                case "20":
                    Console.WriteLine("Get people with maximum payment and latest graduation date:");
                    OutputPeople(_personService.GetPeopleByMaxPaymentAndGraduation());
                    break;
                case "q":
                    escape = true;
                    break;
